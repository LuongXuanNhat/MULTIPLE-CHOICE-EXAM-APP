...
namespace PhanMemThiTracNghiem
{
    public partial class frmThi : Form
    {
        ...
        private void frmThi_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < danhMucCauHoiBAL.GetCauHoi().Count(); i++)
            {
        ...
        private GroupBox TaoCauHoi(CHITIETDETHI i)
        {
            ...
            groupBox.Text = "Câu " + i.MACAUHOI;
            groupBox.Name = i.MACAUHOI.ToString();
            ...
            label.Name = i.MACAUHOI.ToString();
            label.Font = new Font("Be Vietnam Pro", 10);

            foreach (var item in cauHoiBAL.GetThongTinCauHoi().ToList())
            {
                if (item.MACAUHOI == i.MACAUHOI)
                {
                    label.Text = item.NDCAUHOI;
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        if (j == 0)
                        {
                            rdo.Text = item.DAPAN1;
                        }
                        if (j == 1)
                        {
                            rdo.Text = item.DAPAN2;
                        }
                        if (j == 2)
                        {
                            rdo.Text = item.DAPAN3;
                        }
                        if (j == 3)
                        {
                            rdo.Text = item.DAPAN4;
                        }

                        groupBox.Controls.Add(rdo);

                        rdo.Click += (sender, EventArgs) => { buttonNext_Click(sender, EventArgs, groupBox.Name, rdo, item.MACAUHOI); };
        ...
        private void NopBai_Click()
        {
            ...
            float diemThi = 0;
            int demSoCauDung = 0;
            int soCauHoi = danhMucCauHoiBAL.GetCauHoi().Count();
...
//DateTime thoiGianBatDau = (DateTime)i.THOIGIANBD;
//i.DIEM = diemThi;
//i.THOIGIANKT = thoiGianThi;
//i.THOIGIANTHI = (thoiGianThi.Hour * 60 + thoiGianThi.Minute) - (thoiGianBatDau.Hour * 60 + thoiGianBatDau.Minute);